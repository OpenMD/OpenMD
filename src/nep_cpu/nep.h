...
public:
  ...
  // PREPAIR_LOOP: accumulate radial descriptor contribution from one pair
  // q_out[0..n_max_radial] are incremented in place
  void accum_radial(int t1, int t2, double d12, double* q_out);

  // PREPAIR_LOOP: accumulate angular descriptor contribution from one pair
  // sum_fxyz_out[(n_max_angular+1)*NUM_OF_ABC] is incremented in place
  // r12[3] is the vector from atom1 to atom2
  void accum_angular(int t1, int t2, double d12, const double* r12,
                     double* sum_fxyz_out);

  // Between loops: run ANN for one atom given accumulated descriptors
  // q_radial[n_max_radial+1], sum_fxyz[(n_max_angular+1)*NUM_OF_ABC]
  // Produces: energy (eV), Fp_out[annmb.dim] (dE/dq * q_scaler)
  void run_ann_one_atom(int t1, const double* q_radial, const double* sum_fxyz,
                        double& energy, double* Fp_out);

  // PAIR_LOOP: compute combined radial force on atom1 from pair (atom1, atom2)
  // Accounts for atom1-as-center AND atom2-as-center contributions.
  // f12_out[3] is incremented (not overwritten).
  void calc_radial_force(int t1, int t2, double d12, const double* r12,
                         const double* Fp1, const double* Fp2,
                         double* f12_out);

  // PAIR_LOOP: compute combined angular force on atom1 from pair (atom1, atom2)
  // Fp1_ang/Fp2_ang are the angular portions of Fp (length paramb.dim_angular)
  // sum_fxyz1/sum_fxyz2 are per-atom [(n_max_angular+1)*NUM_OF_ABC] arrays
  // f12_out[3] is incremented (not overwritten).
  void calc_angular_force(int t1, int t2, double d12, const double* r12,
