...
namespace OpenMD {
  namespace RNEMD {
    ...
    RNEMD::RNEMD(SimInfo* info) : info_(info),
      ...
      evaluatorA_(info_), evaluatorB_(info_),
      seleManA_(info_),seleManB_(info_),
      ...
      usePeriodicBoundaryConditions_ = simParams->getUsePeriodicBoundaryConditions();
      ...
      if (rnemdParams->haveFluxType()) {
        ...
        for (std::map<std::string, RNEMDFluxType>::iterator fluxStrIter = stringToFluxType.begin();
      ...
      if (hasSelectionA_) {
        ...
        if (usePeriodicBoundaryConditions_) {
          slabWidth_ = hasSlabWidth ? rnemdParams->getSlabWidth()
                                    : hmat_(rnemdPrivilegedAxis_,rnemdPrivilegedAxis_) / 10.0;

          slabACenter_ = hasSlabACenter ? rnemdParams->getSlabACenter()
                                        : 0.0;
      ...
      if (hasSelectionB_) {
        ...
        if (usePeriodicBoundaryConditions_) {
          ...
          slabBCenter_ = hasSlabBCenter ? rnemdParams->getSlabBCenter()
    ...
    void RNEMD::doRNEMD() {
      ...
      // Target exchange quantities (in each exchange) = flux * dividingArea * dt
      // flux = target flux
      ...
      for (unsigned int i = 0; i < nBins_; i++) {
        ...
        if (outputMask_[ACTIVITY]) {
          for (unsigned int j = 0; j < outputTypes_.size(); j++) {
            nden[j] = (binTypeCounts[i][j] / binVolume) * Constants::concentrationConvert;
        ...
        if (binCount[i] > 0) {
          ...
          if (outputMask_[TEMPERATURE]) {
            temp = 2.0 * binKE[i] / (binDOF[i] * Constants::kb * Constants::energyConvert);
    ...
    void RNEMD::setMomentumFluxVector(const std::vector<RealType>& momentumFluxVector) {
    ...
    void RNEMD::setAngularMomentumFluxVector(const std::vector<RealType>& angularMomentumFluxVector) {
    ...
    std::string RNEMD::setSelection(RealType& slabCenter) {
      ...
      selectionStream << "select wrapped" << rnemdAxisLabel_ << " >= " << leftSlabBoundary;

      if (leftSlabBoundary > rightSlabBoundary)
        selectionStream <<  " || wrapped" << rnemdAxisLabel_ << " < " << rightSlabBoundary;
      else
        selectionStream <<  " && wrapped" << rnemdAxisLabel_ << " < " << rightSlabBoundary;
