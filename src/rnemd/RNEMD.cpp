...
RNEMD::RNEMD(SimInfo* info)
    : info_(info),
      ...
      commonA_(info_),
      commonB_(info_),
      outputEvaluator_(info_),
      outputSeleMan_(info_) {
  trialCount_ = 0;
  failTrialCount_ = 0;
  failRootCount_ = 0;
  ...
  // Exchange time should not be less than time step and should be a multiple of
  // dt
...
void RNEMD::collectData() {
  ...
  for (mol = info_->beginMolecule(miter); mol != NULL;
    ...
    for (sd = mol->beginIntegrableObject(iiter); sd != NULL;
      ...
      // Calculate the electric field (kcal/mol/e/Angstrom) for all atoms in the
      // box
  ...
  for (unsigned int i = 0; i < nBins_; i++) {
    ...
    if (outputMask_[ELECTROSTATICPOTENTIAL]) {
      if (usePeriodicBoundaryConditions_ && binEFieldCount[i] > 0) {
        ...
        dynamic_cast<Accumulator*>(data_[ELECTROSTATICPOTENTIAL].accumulator[i])
            ->add(ePot);
      }
    }
    ...
    if (outputMask_[ACTIVITY]) {
      for (unsigned int j = 0; j < outputTypes_.size(); j++) {
        nden[j] =
            (binTypeCounts[i][j] / binVolume) * Constants::concentrationConvert;
...
void RNEMD::writeOutputFile() {
  ...
  if (worldRank == 0) {
    ...
    rnemdFile_ << "#          kinetic = "
               << kineticTarget_ / Constants::energyConvert << " (kcal/mol)\n";
