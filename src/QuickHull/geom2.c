...
realT qh_determinant (realT **rows, int dim, boolT *nearzero) {
  ...
  *nearzero= False;
  if (dim < 2) {
    ...
    det= det3_(rows[0][0], rows[0][1], rows[0][2], rows[1][0], rows[1][1], rows[1][2], rows[2][0], rows[2][1], rows[2][2]);
...
/*-<a                             href="qh-geom.htm#TOC"
  ...
  notes:
    ...
    please use sqrt(dim) since one vector is normalized
...
/*-<a                             href="qh-geom.htm#TOC"
  ...
  design:
    for each point
      for each coordinate
        please use row[dim] to compute partial inner product
      for each coordinate
