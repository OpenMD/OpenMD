...
bool EndCriteria::checkMaxIterations(const size_t iteration,
  ...
  if (iteration < maxIterations_) return false;
  ecType = MaxIterations;
  return true;
}

bool EndCriteria::checkStationaryPoint(const RealType xOld, const RealType xNew,
...
bool EndCriteria::checkStationaryFunctionAccuracy(
    const RealType f, const bool positiveOptimization,
    EndCriteria::Type& ecType) const {
  if (!positiveOptimization) return false;
  if (f >= functionEpsilon_) return false;
...
bool EndCriteria::checkZeroGradientNorm(const RealType gradientNorm,
                                        EndCriteria::Type& ecType) const {
  if (gradientNorm >= gradientNormEpsilon_) return false;
...
// Inspectors
size_t EndCriteria::maxIterations() const { return maxIterations_; }

size_t EndCriteria::maxStationaryStateIterations() const {
  return maxStationaryStateIterations_;
}

RealType EndCriteria::rootEpsilon() const { return rootEpsilon_; }

RealType EndCriteria::functionEpsilon() const { return functionEpsilon_; }
