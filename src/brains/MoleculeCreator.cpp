...
Molecule* MoleculeCreator::createMolecule(ForceField* ff,
                                          MoleculeStamp* molStamp, int stampId,
                                          int globalIndex,
  ...
  for (int i = 0; i < nInversions; ++i) {
    ...
    inversion = createInversion(ff, mol, currentInversionStamp, localIndexMan);
    if (inversion != NULL) {
...
Bend* MoleculeCreator::createBend(ForceField* ff, Molecule* mol,
  ...
  if (bendAtoms.size() == 3) {
    ...
    if (stamp->hasOverride()) {
      ...
      bendType =
          ff->getBendType(normalAtom->getType(), ghostAtom->getType(), "GHOST");
...
CutoffGroup* MoleculeCreator::createCutoffGroup(
    Molecule* mol, CutoffGroupStamp* stamp, LocalIndexManager* localIndexMan) {
  int nAtoms;
  CutoffGroup* cg;
  Atom* atom;
  cg = new CutoffGroup();
