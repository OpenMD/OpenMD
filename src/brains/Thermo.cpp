...
#ifdef IS_MPI
  MPI_Allreduce(MPI_IN_PLACE, &Jc[0], 3, MPI_REALTYPE, MPI_SUM, MPI_COMM_WORLD);
...
Vector3d Thermo::getSystemDipole() {
  ...
  if (!snap->hasSystemDipole) {
    ...
    MPI_Allreduce(MPI_IN_PLACE, dipoleVector.getArrayPointer(), 3, MPI_REALTYPE,
                  MPI_SUM, MPI_COMM_WORLD);
#endif
...
Vector3d Thermo::getCom() {
  ...
  if (!snap->hasCOM) {
    ...
    MPI_Allreduce(MPI_IN_PLACE, &totalMass, 1, MPI_REALTYPE, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, com.getArrayPointer(), 3, MPI_REALTYPE, MPI_SUM,
                  MPI_COMM_WORLD);
#endif
