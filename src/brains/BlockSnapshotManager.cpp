...
BlockSnapshotManager::BlockSnapshotManager(SimInfo* info,
  ...
  std::cout << "            Bytes per Frame:\t" << (unsigned long)bytesPerFrame
            << std::endl;
  std::cout << "             Frame Capacity:\t" << (unsigned long)frameCapacity
            << std::endl;
...
int BlockSnapshotManager::getFirstZeroRefBlock() {
  ...
  return i != activeRefCount_.end() ? activeBlocks_[i - activeRefCount_.begin()]
                                    : -1;
}
