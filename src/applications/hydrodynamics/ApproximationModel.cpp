 ...
 * Rigid Macromolecules", Macromolecules 1980 13 (4), 960-964   //(center of
 *resistance and center of diffusion)
 *
 **/
 ...
 check bead (%lu) and/or bead (%lu).\n",
      ...
      if (i != j) {  // non-self interaction: divided in overlapping and
        ...
        if (rij >=
          ...
          std::cout << "There is overlapping between beads: (" << i << ") and ("
                    << j << ")" << std::endl;
  ...
  Mat3x3d Ddrr;  // translation-rotation coupling diffusion tensor at diffusion
  ...
  Xicomrr =
      Xiorr - UoCOM * Xiott * UoCOM + Xiotr * UoCOM - UoCOM * Xiotr.transpose();
