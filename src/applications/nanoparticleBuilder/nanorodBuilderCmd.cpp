/*
  ...
  /opt/local/bin/gengetopt -C --input=nanorodBuilder.ggo --no-handle-error
  --include-getopt --show-required --unamed-opts --file-name=nanorodBuilderCmd
...
const char *gengetopt_args_info_purpose =
    "Builds pill-shaped, ellipsoidal, or pentagonal nanoparticles and outputs "
    "an\nOpenMD "
    "startup file";
...
const char *gengetopt_args_info_help[] = {
    ...
    "      --latticeConstant=DOUBLE  Lattice spacing in Angstroms for cubic "
    "lattice.\n   "
    "                               (mandatory)",
    ...
    "          "
    "                    (default=off)",
    "      --shellRadius=DOUBLE      Radius containing within it only "
    "molecules of a\n   "
    ...
    "          "
    " template file.",
    "      --vacancyPercent=DOUBLE   Percentage of atoms to remove from "
    "within\n         "
    "                         vacancy range",
    "      --vacancyInnerRadius=DOUBLE\n                                Radius "
    "arround "
    "core-shell where vacancies\n                                  should be "
    "located.",
...
int cmdline_parser_dump(FILE *outfile, struct gengetopt_args_info *args_info) {
  ...
  if (args_info->ellipsoid_given) write_into_file(outfile, "ellipsoid", 0, 0);
  write_multiple_into_file(outfile, args_info->shellRadius_given, "shellRadius",
  ...
  if (args_info->vacancyPercent_given)
    write_into_file(outfile, "vacancyPercent", args_info->vacancyPercent_orig,
                    0);
  if (args_info->vacancyInnerRadius_given)
    write_into_file(outfile, "vacancyInnerRadius",
                    args_info->vacancyInnerRadius_orig, 0);
  if (args_info->vacancyOuterRadius_given)
    write_into_file(outfile, "vacancyOuterRadius",
                    args_info->vacancyOuterRadius_orig, 0);
...
int cmdline_parser_internal(int argc, char **argv,
  ...
  while (1) {
    ...
    c = custom_getopt_long(argc, argv, "hVo:e", long_options, &option_index);
    ...
    switch (c) {
      ...
      case 'e': /* Build an Ellipsoid instead of a rod..  */
        ...
        if (update_arg((void *)&(args_info->ellipsoid_flag), 0,
                       &(args_info->ellipsoid_given),
                       &(local_args_info.ellipsoid_given), optarg, 0, 0,
                       ARG_FLAG, check_ambiguity, override, 1, 0, "ellipsoid",
                       'e', additional_error))
          goto failure;
