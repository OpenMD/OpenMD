...
namespace OpenMD {
  ...
  void BondOrderParameter::collectHistogram(std::vector<RealType> q,
    ...
    for (int l = 0; l <= lMax_; l++) {
      if (q[l] >= MinQ_ && q[l] < MaxQ_) {
        ...
        snprintf(painCave.errMsg, MAX_SIM_ERROR_MSG_LENGTH, "q_l value outside reasonable range\n");
  ...
  void BondOrderParameter::writeOrderParameter(std::vector<RealType> Q,
    ...
    if (osq.is_open()) {
      ...
      snprintf(painCave.errMsg, MAX_SIM_ERROR_MSG_LENGTH, "BondOrderParameter: unable to open %s\n",
