...
void Hxy::process() {
  ...
  for (int istep = 0; istep < nFrames; istep += step_) {
    ...
    in1 =
        (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * (nBinsX_ * nBinsY_));
    out1 =
        (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * (nBinsX_ * nBinsY_));
    in2 =
        (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * (nBinsX_ * nBinsY_));
    ...
    for (unsigned int i = 0; i < nBinsX_; i++) {
      for (unsigned int j = 0; j < nBinsY_; j++) {
        newindex = i * nBinsY_ + j;
        mag1[newindex] =
            sqrt(pow(c_re(out1[newindex]), 2) + pow(c_im(out1[newindex]), 2)) /
            (RealType(nBinsX_ * nBinsY_));
        mag2[newindex] =
            sqrt(pow(c_re(out2[newindex]), 2) + pow(c_im(out2[newindex]), 2)) /
            (RealType(nBinsX_ * nBinsY_));
      }
    }
