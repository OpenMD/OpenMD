...
void TetrahedralityParamXYZ::process() {
  ...
  cerr << "gw = " << gaussWidth_ << " vS = " << voxelSize_ << " kMax = " << kMax
       << " kSqLim = " << kSqLim << "\n";
    ...
    for (sd = seleMan1_.beginSelected(isd1); sd != NULL;
      ...
      for (int i = 0; i < nbors - 1; i++) {
        ...
        for (int j = i + 1; j < nbors; j++) {
          ...
          rj = sdj->getPos();
          rkj = rk - rj;
          if (usePeriodicBoundaryConditions_) currentSnapshot_->wrapVector(rkj);
          rkj.normalize();
      ...
      if (nang > 0) {
        ...
        Vector3i whichVoxel(int(pos[0] / voxelSize_), int(pos[1] / voxelSize_),
