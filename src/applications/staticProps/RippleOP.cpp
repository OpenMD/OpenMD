...
namespace OpenMD {
  ...
  RippleOP::RippleOP(SimInfo* info, const std::string& filename,
    ...
    if (!evaluator1_.isDynamic()) {
      ...
      snprintf(painCave.errMsg, MAX_SIM_ERROR_MSG_LENGTH, "--sele1 must be static selection\n");
    ...
    if (!evaluator2_.isDynamic()) {
      ...
      snprintf(painCave.errMsg, MAX_SIM_ERROR_MSG_LENGTH, "--sele2 must be static selection\n");
      painCave.severity = OPENMD_ERROR;
      painCave.isFatal  = 1;
      simError();
    }
