...
namespace OpenMD {
  ...
  MomentumHistogram::MomentumHistogram(SimInfo* info, const std::string& filename,
	     const std::string& sele, int nbins, int momentum_type, int momentum_component)
    : StaticAnalyser(info, filename, nbins), selectionScript_(sele),
      evaluator_(info), seleMan_(info), nBins_(nbins), mom_type_(momentum_type), mom_comp_(momentum_component) {
  ...
  void MomentumHistogram::write() {
    ...
    if (rdfStream.is_open()) {
      rdfStream << "#" <<momentum_name_<< component_name_[mom_comp_]<<" distribtution \n";
