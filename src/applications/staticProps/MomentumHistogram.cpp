...
namespace OpenMD {
  ...
  void MomentumHistogram::process() {
    ...
    if (momentum.empty()) {
      snprintf(painCave.errMsg, MAX_SIM_ERROR_MSG_LENGTH, "Momentum for selected atom not found.\n");
      painCave.isFatal = 1;
      simError();
    }
  ...
  void MomentumHistogram::write() {
    ...
    if (rdfStream.is_open()) {
      ...
      snprintf(painCave.errMsg, MAX_SIM_ERROR_MSG_LENGTH, "MomentumHistogram: unable to open %s\n",
