...
namespace OpenMD {
COHZ::COHZ(SimInfo* info, const std::string& filename, const std::string& sele1,
           const std::string& sele2, int order, int nZbins, int axis)
    : MoleculeACF<Vector<RealType, 4>>(
...
void COHZ::writeCorrelate() {
  ...
  if (ofs1.is_open()) {
    ...
    if (!paramString_.empty()) ofs1 << "# parameters: " << paramString_ << "\n";
    ...
    sprintf(painCave.errMsg, "COHz::writeCorrelate Error: failed to open %s\n",
            Dfile.c_str());
    painCave.isFatal = 1;
    simError();
  }
  ...
  if (ofs2.is_open()) {
    ...
    if (!paramString_.empty()) ofs2 << "# parameters: " << paramString_ << "\n";
  ...
  if (ofs3.is_open()) {
    ...
    if (!paramString_.empty()) ofs3 << "# parameters: " << paramString_ << "\n";
