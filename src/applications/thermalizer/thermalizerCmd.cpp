/*
  ...
  gengetopt --include-getopt --long-help --show-required --unamed-opts
  --file-name=thermalizerCmd --c-extension=cpp --header-extension=hpp
...
const char* gengetopt_args_info_purpose =
    "Two functionalities: 1. (-t) Resamples the velocities for all the "
    "integrable\nobjects in an OpenMD file from a Maxwell-Boltzmann "
    "distribution. 2. "
    "(-e) Scales\nthe total energy of all integrable objects in an OpenMD file "
    "to a "
    "desired total\nenergy.";
...
int cmdline_parser_dump(FILE* outfile, struct gengetopt_args_info* args_info) {
  ...
  if (args_info->chargetemperature_given)
    write_into_file(outfile, "chargetemperature",
                    args_info->chargetemperature_orig, 0);
...
static void reset_group_thermalizer(struct gengetopt_args_info* args_info) {
  if (!args_info->thermalizer_group_counter) return;
...
int cmdline_parser_required2(struct gengetopt_args_info* args_info,
  ...
  if (args_info->thermalizer_group_counter == 0) {
    fprintf(
        stderr,
        "%s: %d options of group thermalizer were given. One is required%s.\n",
        prog_name, args_info->thermalizer_group_counter,
        (additional_error ? additional_error : ""));
    error_occurred = 1;
  }
...
int cmdline_parser_internal(int argc, char** argv,
  ...
  while (1) {
    ...
    c = custom_getopt_long(argc, argv, "hVi:o:t:e:c:", long_options,
                           &option_index);
    ...
    case 't': /* temperature (K).  */
      ...
      if (update_arg((void*)&(args_info->temperature_arg),
                     ...
                     &(local_args_info.temperature_given), optarg, 0, 0,
                     ARG_DOUBLE, check_ambiguity, override, 0, 0, "temperature",
                     't', additional_error))
        goto failure;

      break;
    case 'e': /* energy (kcal/mol).  */

      if (args_info->thermalizer_group_counter && override)
        reset_group_thermalizer(args_info);
      args_info->thermalizer_group_counter += 1;

      if (update_arg((void*)&(args_info->energy_arg), &(args_info->energy_orig),
                     &(args_info->energy_given),
                     &(local_args_info.energy_given), optarg, 0, 0, ARG_DOUBLE,
                     check_ambiguity, override, 0, 0, "energy", 'e',
                     additional_error))
        goto failure;
    ...
    case 'c': /* charge temperature (K).  */
      ...
      if (update_arg((void*)&(args_info->chargetemperature_arg),
                     &(args_info->chargetemperature_orig),
                     &(args_info->chargetemperature_given),
                     &(local_args_info.chargetemperature_given), optarg, 0, 0,
                     ARG_DOUBLE, check_ambiguity, override, 0, 0,
                     "chargetemperature", 'c', additional_error))
        goto failure;
  ...
  if (args_info->thermalizer_group_counter > 1) {
    fprintf(
        stderr,
        "%s: %d options of group thermalizer were given. One is required%s.\n",
        argv[0], args_info->thermalizer_group_counter,
        (additional_error ? additional_error : ""));
    error_occurred = 1;
  }
