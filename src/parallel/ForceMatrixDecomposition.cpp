...
void ForceMatrixDecomposition::zeroWorkArrays() {
  ...
  if (storageLayout_ & DataStorage::dslParticlePot) {
    fill(snap_->atomData.particlePot.begin(), snap_->atomData.particlePot.end(),
         0.0);
  }
...
void ForceMatrixDecomposition::distributeData() {
  ...
  AtomPlanVectorColumn->gather(snap_->atomData.position, atomColData.position);
  ...
  if (storageLayout_ & DataStorage::dslFlucQPosition) {
    ...
    AtomPlanRealColumn->gather(snap_->atomData.flucQPos, atomColData.flucQPos);
  }
...
void ForceMatrixDecomposition::collectIntermediateData() {
  ...
  if (storageLayout_ & DataStorage::dslDensity) {
    ...
    for (int i = 0; i < n; i++) snap_->atomData.density[i] += rho_tmp[i];
  }
...
void ForceMatrixDecomposition::distributeIntermediateData() {
  ...
  if (storageLayout_ & DataStorage::dslFunctional) {
    AtomPlanRealRow->gather(snap_->atomData.functional, atomRowData.functional);
...
void ForceMatrixDecomposition::collectData() {
  ...
  if (storageLayout_ & DataStorage::dslTorque) {
    ...
    for (int i = 0; i < nt; i++) snap_->atomData.torque[i] += trq_tmp[i];
  }
  ...
  if (storageLayout_ & DataStorage::dslFlucQForce) {
    ...
    for (int i = 0; i < nq; i++) snap_->atomData.flucQFrc[i] += fqfrc_tmp[i];
  }
  ...
  if (storageLayout_ & DataStorage::dslSitePotential) {
    ...
    for (int i = 0; i < nsp; i++) snap_->atomData.sitePotential[i] += sp_tmp[i];
  }
...
#ifdef IS_MPI
  MPI_Allreduce(MPI_IN_PLACE, &selfPot[0], N_INTERACTION_FAMILIES, MPI_REALTYPE,
