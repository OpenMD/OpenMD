/*
  ...
  /opt/local/bin/gengetopt -C --input=SequentialProps.ggo --no-handle-error --include-getopt --show-required --unamed-opts --file-name=SequentialPropsCmd --c-extension=cpp --header-extension=hpp --no-handle-error --include-getopt --show-required --unamed-opts --file-name=SequentialPropsCmd --c-extension=cpp --header-extension=hpp
...
const char *gengetopt_args_info_purpose = "Computes a time history of static properties from a dump file.";
  ...
  while (1)
    {
      ...
      c = custom_getopt_long (argc, argv, "hVi:o:b:x:y:z:r:c", long_options, &option_index);
      ...
      switch (c)
        ...
        case 0:	/* Long option with no short option */
          ...
          /* compute length of nanorod.  */
          else if (strcmp (long_options[option_index].name, "nanolength") == 0)
          {
            ...
            if (update_arg( 0 , 
                 0 , &(args_info->nanolength_given),
                &(local_args_info.nanolength_given), optarg, 0, 0, ARG_NO,
                check_ambiguity, override, 0, 0,
                "nanolength", '-',
                additional_error))
              goto failure;
          ...
          /* compute volume and surface area of hull.  */
          else if (strcmp (long_options[option_index].name, "nanovolume") == 0)
          {
