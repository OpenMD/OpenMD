...
ZconstraintForceManager::ZconstraintForceManager(SimInfo* info)
  ...
  if (simParam->haveDt()) {
    ...
    sprintf(painCave.errMsg, "ZconstraintForceManager Error: dt is not set\n");
    painCave.isFatal = 1;
    simError();
  }
...
ZconstraintForceManager::~ZconstraintForceManager() { delete fzOut; }
...
bool ZconstraintForceManager::isZMol(Molecule* mol) {
  return allZMolIndices_.find(mol->getGlobalIndex()) == allZMolIndices_.end()
             ? false
             : true;
}
...
#ifdef IS_MPI
  MPI_Allreduce(MPI_IN_PLACE, &haveFixed, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
#endif
...
#ifdef IS_MPI
  MPI_Allreduce(MPI_IN_PLACE, &haveMoving, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
#endif
