...
namespace antlr {
   ...
   *Turing machine instruction streams (programs) operating on a single input
   *tape. :)
   *
   *	 Since the operations are done lazily at toString-time, operations do
   *not screw up the token index values.  That is, an insert operation at token
   *	 index i does not change the index values for tokens i+1..n-1.
   *
   *	 Because operations never actually alter the buffer, you may always get
   *	 the original token stream back without undoing anything.  Since
   *	 the instructions are queued up, you can easily simulate transactions
   *and roll back any changes if there is an error just by removing
   ...
   *instruction streams and use that name again when printing the buffer.
   *This could be useful for generating a C file and also its header file--all
...
#else
  enum { MIN_TOKEN_INDEX = 0, PROGRAM_INIT_SIZE = 100 };
#endif
    ...
    public:
      ...
      virtual size_t getIndex() const { return index; }
      virtual const char* type() const { return "RewriteOperation"; }
    ...
    struct executeOperation {
      ...
      void operator()(RewriteOperation* t) { t->execute(out); }
    };
    ...
    typedef ANTLR_USE_NAMESPACE(
        std) map<ANTLR_USE_NAMESPACE(std) string, operation_list> program_map;

    class InsertBeforeOp : public RewriteOperation {
    public:
      InsertBeforeOp(size_t index,
      ...
      virtual const char* type() const { return "InsertBeforeOp"; }
    };

    class ReplaceOp : public RewriteOperation {
    public:
      ReplaceOp(size_t from, size_t to, ANTLR_USE_NAMESPACE(std) string text) :
          RewriteOperation(from, text), lastIndex(to) {}
      virtual ~ReplaceOp() {}
      virtual size_t execute(ANTLR_USE_NAMESPACE(std) ostream& out) {
        out << text;
        return lastIndex + 1;
      }
      virtual const char* type() const { return "ReplaceOp"; }
    ...
    public:
      DeleteOp(size_t from, size_t to) : ReplaceOp(from, to, "") {}
      virtual const char* type() const { return "DeleteOp"; }
    };
    ...
    void insertBefore(RefTokenWithIndex t,
                      const ANTLR_USE_NAMESPACE(std) string& text) {
      // std::cout << "insertBefore index " << t->getIndex() << " " << text <<
      // std::endl;
    ...
    void replace(RefTokenWithIndex indexT,
                 const ANTLR_USE_NAMESPACE(std) string& text) {
      replace(DEFAULT_PROGRAM_NAME, indexT->getIndex(), indexT->getIndex(),
              text);
    }

    void replace(RefTokenWithIndex from, RefTokenWithIndex to,
                 const ANTLR_USE_NAMESPACE(std) string& text) {
      replace(DEFAULT_PROGRAM_NAME, from, to, text);
    }
    ...
    void replace(const ANTLR_USE_NAMESPACE(std) string& programName,
                 ...
                 const ANTLR_USE_NAMESPACE(std) string& text) {
      replace(programName, from->getIndex(), to->getIndex(), text);
    }

    void remove(size_t index) { remove(DEFAULT_PROGRAM_NAME, index, index); }
    ...
    void remove(const ANTLR_USE_NAMESPACE(std) string& programName, size_t from,
                size_t to) {
    ...
    void discard(int ttype) { discardMask.add(ttype); }

    RefToken getToken(size_t i) { return RefToken(tokens.at(i)); }

    size_t getTokenStreamSize() const { return tokens.size(); }

    void originalToStream(ANTLR_USE_NAMESPACE(std) ostream& out) const {
      ANTLR_USE_NAMESPACE(std)
      for_each(tokens.begin(), tokens.end(), tokenToStream(out));
    }

    void originalToStream(ANTLR_USE_NAMESPACE(std) ostream& out, size_t start,
                          size_t end) const;
    ...
    void toStream(ANTLR_USE_NAMESPACE(std) ostream& out, size_t start,
                  size_t end) const {
    ...
    void toDebugStream(ANTLR_USE_NAMESPACE(std) ostream& out, size_t start,
                       size_t end) const;
     ...
     */
    size_t getLastRewriteTokenIndex(const ANTLR_USE_NAMESPACE(std)
                                        string& programName) const {
      program_map::const_iterator rewrites = programs.find(programName);

      if (rewrites == programs.end()) return 0;
  ...
  protected:
    ...
    void addToSortedRewriteList(const ANTLR_USE_NAMESPACE(std)
                                    string& programName,
                                RewriteOperation* op);
