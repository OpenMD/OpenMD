...
ASTFactory::ASTFactory()
    : default_factory_descriptor(ANTLR_USE_NAMESPACE(std) make_pair(
          CommonAST::TYPE_NAME, &CommonAST::factory)) {
...
ASTFactory::ASTFactory(const char* factory_node_name, factory_type fact)
    : default_factory_descriptor(ANTLR_USE_NAMESPACE(std)
                                     make_pair(factory_node_name, fact)) {
  nodeFactories.resize(Token::MIN_USER_TYPE, &default_factory_descriptor);
}
...
ASTFactory::~ASTFactory() {
  ...
  while (i != nodeFactories.end()) {
    if (*i != &default_factory_descriptor) delete *i;
    ++i;
  }
}

/// Register a factory for a given AST type
void ASTFactory::registerFactory(int type, const char* ast_name,
  ...
  // And add new thing..
  nodeFactories[type] = new ANTLR_USE_NAMESPACE(std)
      pair<const char*, factory_type>(ast_name, factory);
}
...
RefAST ASTFactory::create(const ANTLR_USE_NAMESPACE(std) string& type_name,
...
RefAST ASTFactory::create(RefToken tok) {
  //	cout << "create( tok="<< tok->getType() << ", " << tok->getText() << ")"
  //<<
  // nodeFactories.size() << endl;
...
RefAST ASTFactory::dupTree(RefAST t) {
  ...
  if (t) result->setFirstChild(dupList(t->getFirstChild()));
  return result;
}
...
RefAST ASTFactory::make(ANTLR_USE_NAMESPACE(std) vector<RefAST>& nodes) {
  if (nodes.size() == 0) return RefAST(nullASTptr);
  ...
  if (root)
    root->setFirstChild(
        RefAST(nullASTptr));  // don't leave any old pointers set
  ...
  for (unsigned int i = 1; i < nodes.size(); i++) {
    ...
    if (tail)  // RK: I cannot fathom why this missing check didn't bite anyone
               // else...
    {
      // Chase tail to last sibling
      while (tail->getNextSibling()) tail = tail->getNextSibling();
    }
  }
...
RefAST ASTFactory::LoadAST(ANTLR_USE_NAMESPACE(std) istream& infile) {
  ...
  if (!infile.get(ch)) return nullAST;
