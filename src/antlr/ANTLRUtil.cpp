...
istream& eatwhite(ANTLR_USE_NAMESPACE(std) istream& is) {
  char c;
  while (is.get(c)) {
...
string read_string(ANTLR_USE_NAMESPACE(std) istream& in) {
  ...
  while (state != FINISHED && in.get(ch)) {
    switch (state) {
      case START:
        ...
        if (ch != '"') throw IOException("string must start with '\"'");
        state = READING;
        continue;
      case READING:
  ...
  if (state != FINISHED) throw IOException("badly formatted string: " + ret);
...
string read_identifier(ANTLR_USE_NAMESPACE(std) istream& in) {
  ...
  while (in.get(ch)) {
    ...
    if (ANTLR_USE_NAMESPACE(std) isupper(ch) ||
        ...
        ANTLR_USE_NAMESPACE(std) isdigit(ch) || ch == '_')
#else
