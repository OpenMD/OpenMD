...
namespace antlr {
  ...
  public:
     ...
     */
    virtual void setASTFactory(ASTFactory* factory) { astFactory = factory; }
    /** Return a pointer to the ASTFactory used.
     * So you might use it in subsequent treewalkers or to reload AST's
     * from disk.
     */
    virtual ASTFactory* getASTFactory() { return astFactory; }
    ...
    virtual void match(int t) {
      if (DEBUG_PARSER) {
        ...
        cout << "enter match(" << t << ") with LA(1)=" << LA(1)
      ...
      if (LA(1) != t) {
        if (DEBUG_PARSER) {
          traceIndent();
          ANTLR_USE_NAMESPACE(std)
          cout << "token mismatch: " << LA(1) << "!=" << t
               << ANTLR_USE_NAMESPACE(std) endl;
        }
        throw MismatchedTokenException(getTokenNames(), getNumTokens(), LT(1),
                                       t, false, getFilename());
      } else {
    ...
    virtual void match(const BitSet& b) {
      ...
      if (!b.member(LA(1))) {
        if (DEBUG_PARSER) {
          traceIndent();
          ANTLR_USE_NAMESPACE(std)
          cout << "token mismatch: " << LA(1) << " not member of "
     ...
     */
    virtual inline unsigned int mark() { return inputState->getInput().mark(); }
  ...
  protected:
    ...
    public:
      Tracer(Parser* p, const char* t) : parser(p), text(t) {
        parser->traceIn(text);
      }
      ~Tracer() {
