...
namespace antlr {
  ...
  public:
    ...
    LexerInputState(InputBuffer* inbuf) :
        column(1), line(1), tokenStartColumn(1), tokenStartLine(1), guessing(0),
        filename(""), input(inbuf), inputResponsible(true) {}
    ...
    LexerInputState(InputBuffer& inbuf) :
        column(1), line(1), tokenStartColumn(1), tokenStartLine(1), guessing(0),
        filename(""), input(&inbuf), inputResponsible(false) {}
    ...
    LexerInputState(ANTLR_USE_NAMESPACE(std) istream& in) :
        column(1), line(1), tokenStartColumn(1), tokenStartLine(1), guessing(0),
        filename(""), input(new CharBuffer(in)), inputResponsible(true) {}
    ...
    virtual void initialize(ANTLR_USE_NAMESPACE(std) istream& in,
      ...
      if (input && inputResponsible) delete input;
  ...
  inline InputBuffer& LexerInputState::getInput() { return *input; }
