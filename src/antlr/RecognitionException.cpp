...
RecognitionException::RecognitionException(const ANTLR_USE_NAMESPACE(std)
                                               string& s)
    : ANTLRException(s), line(-1), column(-1) {}
...
string RecognitionException::getFileLineColumnString() const {
  ...
  if (fileName.length() > 0) fileLineColumnString = fileName + ":";
