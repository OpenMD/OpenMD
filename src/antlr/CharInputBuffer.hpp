...
namespace antlr {
  ...
  /** CharInputBuffer.hpp provides an InputBuffer for plain character arrays
   * (buffers).
   */
  ...
  public:
    ...
    CharInputBuffer(unsigned char* buf, size_t size, bool owner = false) :
        buffer(buf), ptr(buf), end(buf + size), delete_buffer(owner) {}
    ...
    ~CharInputBuffer(void) {
      if (delete_buffer && buffer) delete[] buffer;
    }
    ...
    virtual int getChar(void) { return (ptr < end) ? *ptr++ : EOF; }
