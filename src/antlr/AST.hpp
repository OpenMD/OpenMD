...
namespace antlr {
  ...
  public:
    ...
    virtual void initialize(int t,
                            const ANTLR_USE_NAMESPACE(std) string& txt) = 0;
     ...
     */
    virtual bool attributesToStream(ANTLR_USE_NAMESPACE(std)
                                        ostream& out) const = 0;
     ...
     */
    friend ANTLR_USE_NAMESPACE(std) ostream& operator<<(ANTLR_USE_NAMESPACE(std)
                                                            ostream& output,
                                                        const RefAST& ast);
#endif
...
#ifdef ANTLR_SUPPORT_XML
  inline ANTLR_USE_NAMESPACE(std) ostream& operator<<(ANTLR_USE_NAMESPACE(std)
                                                          ostream& output,
                                                      const RefAST& ast) {
    ast->toStream(output);
    return output;
  ...
  inline bool operator<(RefAST l, RefAST r) {
    return nullAST == l ? (nullAST == r ? false : true) :
                          l->getType() < r->getType();
  }
#endif
