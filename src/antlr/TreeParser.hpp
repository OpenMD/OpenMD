  ...
  public:
    ...
    virtual void match(RefAST t, const BitSet& b) {
      if (!t || t == ASTNULL || !b.member(t->getType()))
        throw MismatchedTokenException(getTokenNames(), getNumTokens(), t, b,
                                       false);
    }
    ...
    virtual ASTFactory* getASTFactory() const { return astFactory; }
  ...
  protected:
    virtual void match(RefAST t, int ttype) {
      if (!t || t == ASTNULL || t->getType() != ttype)
        throw MismatchedTokenException(getTokenNames(), getNumTokens(), t,
                                       ttype, false);
    }
    ...
    public:
      ...
      ~Tracer() { parser->traceOut(text, tree); }
