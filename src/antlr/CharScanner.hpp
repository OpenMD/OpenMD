...
ANTLR_C_USING(strcasecmp)
  ...
  class ANTLR_API CharScannerLiteralsLess :
      public ANTLR_USE_NAMESPACE(std)
          binary_function<ANTLR_USE_NAMESPACE(std) string,
                          ANTLR_USE_NAMESPACE(std) string, bool> {
  private:
  ...
  public:
    ...
    CharScannerLiteralsLess(const CharScanner* theScanner) :
        scanner(theScanner) {}
    bool operator()(const ANTLR_USE_NAMESPACE(std) string& x,
                    const ANTLR_USE_NAMESPACE(std) string& y) const;
  ...
  public:
    ...
    virtual void append(const ANTLR_USE_NAMESPACE(std) string& s) {
      if (saveConsumedInput) text += s;
    }

    virtual void commit() { inputState->getInput().commit(); }

    /** called by the generated lexer to do error recovery, override to
     * customize the behaviour.
     */
    virtual void recover(const RecognitionException& ex,
    ...
    virtual void consumeUntil(const BitSet& set) {
      for (;;) {
        int la_1 = LA(1);
        if (la_1 == EOF_CHAR || set.member(la_1)) break;
        consume();
      }
    }

    /// Mark the current position and return a id for it
    virtual unsigned int mark() { return inputState->getInput().mark(); }
    ...
    virtual void match(int c) {
      int la_1 = LA(1);
      if (la_1 != c) throw MismatchedCharException(la_1, c, false, this);
      consume();
    }
    ...
    virtual void match(const BitSet& b) {
      ...
      if (!b.member(la_1)) throw MismatchedCharException(la_1, b, false, this);
      consume();
    }
    ...
    virtual void matchNot(int c) {
      ...
      if (la_1 == c) throw MismatchedCharException(la_1, c, true, this);
    ...
    virtual bool getCaseSensitive() const { return caseSensitive; }

    virtual void setCaseSensitive(bool t) { caseSensitive = t; }

    virtual bool getCaseSensitiveLiterals() const = 0;

    /// Get the line the scanner currently is in (starts at 1)
    virtual int getLine() const { return inputState->line; }

    /// set the line number
    virtual void setLine(int l) { inputState->line = l; }

    /// Get the column the scanner currently is in (starts at 1)
    virtual int getColumn() const { return inputState->column; }
    /// set the column number
    virtual void setColumn(int c) { inputState->column = c; }
    ...
    virtual bool getCommitToPath() const { return commitToPath; }

    virtual void setCommitToPath(bool commit) { commitToPath = commit; }
    ...
    virtual void setText(const ANTLR_USE_NAMESPACE(std) string& s) { text = s; }
    ...
    virtual RefToken getTokenObject() const { return _returnToken; }
    ...
    virtual InputBuffer& getInputBuffer() { return inputState->getInput(); }

    virtual LexerSharedInputState getInputState() { return inputState; }
    ...
    virtual int testLiteralsTable(int ttype) const {
      ...
      map<ANTLR_USE_NAMESPACE(std) string, int,
          CharScannerLiteralsLess>::const_iterator i = literals.find(text);
      if (i != literals.end()) ttype = (*i).second;
      return ttype;
    }
     ...
     * a token
     */
    virtual int testLiteralsTable(const ANTLR_USE_NAMESPACE(std) string& txt,
                                  int ttype) const {
      ANTLR_USE_NAMESPACE(std)
      map<ANTLR_USE_NAMESPACE(std) string, int,
  ...
  protected:
    ...
    public:
      Tracer(CharScanner* p, const char* t) : parser(p), text(t) {
        parser->traceIn(text);
      }
      ~Tracer() { parser->traceOut(text); }
    };
  ...
  inline bool CharScannerLiteralsLess::operator()(const ANTLR_USE_NAMESPACE(std)
                                                      string& x,
                                                  const ANTLR_USE_NAMESPACE(std)
                                                      string& y) const {
    if (scanner->getCaseSensitiveLiterals())
      return ANTLR_USE_NAMESPACE(std)
          less<ANTLR_USE_NAMESPACE(std) string>()(x, y);
    else {
#ifdef NO_STRCASECMP
