...
namespace antlr {
  ...
  public:
    ...
    //*/ 	ASTPair copy() { 		ASTPair tmp = new ASTPair(); 		tmp.root =
    //root; 		tmp.child = child; 		return tmp;
    //	}
    ANTLR_USE_NAMESPACE(std) string toString() const {
      ANTLR_USE_NAMESPACE(std)
      string r =
          !root ? ANTLR_USE_NAMESPACE(std) string("null") : root->getText();
      ANTLR_USE_NAMESPACE(std)
      string c =
          !child ? ANTLR_USE_NAMESPACE(std) string("null") : child->getText();
      return "[" + r + "," + c + "]";
    }
  };
