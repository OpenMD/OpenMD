...
namespace antlr {
  ...
  namespace {
    ...
    struct compareOperationIndex {
      ...
      bool operator()(const RewriteOperation* a,
                      const RewriteOperation* b) const {
    ...
    struct dumpTokenWithIndex {
      ...
      void operator()(const RefTokenWithIndex& t) {
        out << "[txt='" << t->getText() << "' tp=" << t->getType()
            << " idx=" << t->getIndex() << "]\n";
      }
  ...
  TokenStreamRewriteEngine::TokenStreamRewriteEngine(TokenStream& upstream) :
      stream(upstream), index(MIN_TOKEN_INDEX), tokens(), programs(),
      discardMask() {}

  TokenStreamRewriteEngine::TokenStreamRewriteEngine(TokenStream& upstream,
                                                     size_t initialSize) :
      stream(upstream),
      index(MIN_TOKEN_INDEX), tokens(initialSize), programs(), discardMask() {}
  ...
  void TokenStreamRewriteEngine::rollback(const std::string& programName,
    ...
    if (rewrite != programs.end()) {
      ...
      operation_list::iterator j = prog.begin(), end = prog.end();

      std::advance(j, instructionIndex);
      if (j != end) prog.erase(j, end);
    }
  }
  ...
  void TokenStreamRewriteEngine::toStream(std::ostream& out,
    ...
    if (rewriter == programs.end()) return;

    // get the prog and some iterators in it...
    const operation_list& prog                    = rewriter->second;
    operation_list::const_iterator rewriteOpIndex = prog.begin(),
    ...
    while (tokenCursor <= lastToken) {
      //			std::cout << "tokenCursor = " << tokenCursor <<
      //" first
      // prog index
      //=
      //"
      //<<
      //(*rewriteOpIndex)->getIndex() << std::endl;
      ...
      if (tokenCursor <= lastToken) out << tokens[tokenCursor++]->getText();
    }
    // std::cout << "Handling tail operations # left = " <<
    // std::distance(rewriteOpIndex,rewriteOpEnd) << std::endl; now see if there
    // are operations (append) beyond last token index
    std::for_each(rewriteOpIndex, rewriteOpEnd, executeOperation(out));
    rewriteOpIndex = rewriteOpEnd;
  }

  void TokenStreamRewriteEngine::toDebugStream(std::ostream& out, size_t start,
                                               size_t end) const {
    token_list::const_iterator s = tokens.begin();
    std::advance(s, start);
    token_list::const_iterator e = s;
  ...
  void TokenStreamRewriteEngine::addToSortedRewriteList(
      const std::string& programName, RewriteOperation* op) {
    ...
    if (i != end) {
      operation_list::iterator pos =
          std::upper_bound(i, end, op, compareOperationIndex());
      prog.insert(pos, op);
    } else
      prog.push_back(op);
  }
