...
#define OFFSET_MAX_RESIZE 5000
  ...
  public:
    CircularQueue() : storage(), m_offset(0) {}
    ~CircularQueue() {}
    ...
    inline T elementAt(size_t idx) const { return storage[idx + m_offset]; }
    void removeFirst() {
    ...
    inline void append(const T& t) { storage.push_back(t); }
    inline size_t entries() const { return storage.size() - m_offset; }
