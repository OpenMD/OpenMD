...
void BaseAST::doWorkForFindAll(ANTLR_USE_NAMESPACE(std) vector<RefAST>& v,
  ...
  for (RefAST sibling = this; sibling; sibling = sibling->getNextSibling()) {
    ...
    if (sibling->getFirstChild()) {
      RefBaseAST(sibling->getFirstChild())
...
bool BaseAST::equalsList(RefAST t) const {
  ...
  for (; sibling && t;
    ...
    if (!sibling->equals(t)) return false;
...
bool BaseAST::equalsTree(RefAST t) const {
  ...
  // if roots match, do full list match test on children.
  if (getFirstChild()) {
    if (!getFirstChild()->equalsList(t->getFirstChild())) return false;
  }
  // sibling has no kids, make sure t doesn't either
  else if (t->getFirstChild())
    return false;
...
bool BaseAST::equalsTreePartial(RefAST sub) const {
  ...
  // check roots first
  if (!equals(sub)) return false;
  // if roots match, do full list partial match test on children.
  if (getFirstChild())
    if (!getFirstChild()->equalsListPartial(sub->getFirstChild())) return false;
...
ANTLR_USE_NAMESPACE(std) string BaseAST::toStringList() const {
  ...
  if (getNextSibling()) ts += getNextSibling()->toStringList();
...
bool BaseAST::attributesToStream(ANTLR_USE_NAMESPACE(std) ostream& out) const {
  out << "text=\"" << this->getText() << "\" type=\"" << this->getType()
      << "\"";
...
void BaseAST::toStream(ANTLR_USE_NAMESPACE(std) ostream& out) const {
  for (RefAST node = this; node != 0; node = node->getNextSibling()) {
    ...
    if (need_close_tag) {
      ...
      if (node->getFirstChild() != 0) node->getFirstChild()->toStream(out);
