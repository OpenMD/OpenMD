...
namespace antlr {
  ...
  TokenStreamHiddenTokenFilter::TokenStreamHiddenTokenFilter(
      TokenStream& input) :
    ...
    while (hideMask.member(LA(1)->getType()) ||
      ...
      if (hideMask.member(LA(1)->getType())) {
        if (!p) {
          ...
          static_cast<CommonHiddenStreamToken*>(LA(1).get())
              ->setHiddenBefore(p);  // double-link
          p = LA(1);
        }
        lastHiddenToken = p;
        if (!firstHidden) firstHidden = p;  // record hidden token if first
      }
      consume();
    }
  }
  ...
  BitSet TokenStreamHiddenTokenFilter::getHideMask() const { return hideMask; }
  ...
  void TokenStreamHiddenTokenFilter::hide(int m) { hideMask.add(m); }

  void TokenStreamHiddenTokenFilter::hide(const BitSet& mask) {
    hideMask = mask;
  }

  RefToken TokenStreamHiddenTokenFilter::LA(int) { return nextMonitoredToken; }
  ...
  RefToken TokenStreamHiddenTokenFilter::nextToken() {
    ...
    if (!LA(1)) { consumeFirst(); }

    // we always consume hidden tokens after monitored, thus,
    // upon entry LA(1) is a monitored token.
    RefToken monitored = LA(1);
    // point to hidden tokens found during last invocation
    static_cast<CommonHiddenStreamToken*>(monitored.get())
        ->setHiddenBefore(lastHiddenToken);
