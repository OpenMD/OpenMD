...
DumpWriter::DumpWriter(SimInfo* info)
    : info_(info),
      filename_(info->getDumpFileName()),
...
void DumpWriter::writeFrameProperties(std::ostream& os, Snapshot* s) {
  ...
  sprintf(buffer,
          ...
          "%.10g, "
          "%.10g, %.10g }}\n",
          hmat(0, 0), hmat(1, 0), hmat(2, 0), hmat(0, 1), hmat(1, 1),
          hmat(2, 1), hmat(0, 2), hmat(1, 2), hmat(2, 2));
  os << buffer;
...
std::string DumpWriter::prepareDumpLine(StuntDouble* sd) {
  ...
  sprintf(tempBuffer, "%18.10g %18.10g %18.10g %13e %13e %13e", pos[0], pos[1],
  ...
  if (sd->isDirectional()) {
    ...
    sprintf(tempBuffer, " %13e %13e %13e %13e %13e %13e %13e", q[0], q[1], q[2],
            q[3], ji[0], ji[1], ji[2]);
    line += tempBuffer;
  }
