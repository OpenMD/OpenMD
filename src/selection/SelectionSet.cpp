...
void SelectionSet::flip(std::vector<int> bitIndex) {
  for (int i = 0; i < N_SELECTIONTYPES; i++) bitsets_[i].flip(bitIndex[i]);
}

void SelectionSet::flip(std::vector<int> fromIndex, std::vector<int> toIndex) {
  for (int i = 0; i < N_SELECTIONTYPES; i++)
    bitsets_[i].flip(fromIndex[i], toIndex[i]);
}

void SelectionSet::flip() {
  for (int i = 0; i < N_SELECTIONTYPES; i++) bitsets_[i].flip();
}
...
std::vector<bool> SelectionSet::any() {
  std::vector<bool> result(N_SELECTIONTYPES);
  for (int i = 0; i < N_SELECTIONTYPES; i++) result[i] = bitsets_[i].any();
  return result;
}

std::vector<bool> SelectionSet::none() {
  std::vector<bool> result(N_SELECTIONTYPES);
  for (int i = 0; i < N_SELECTIONTYPES; i++) result[i] = bitsets_[i].none();
  return result;
}
...
void SelectionSet::andOperator(const SelectionSet& ss) {
  for (int i = 0; i < N_SELECTIONTYPES; i++) bitsets_[i] &= ss.bitsets_[i];
}
...
void SelectionSet::clearAll() {
  for (int i = 0; i < N_SELECTIONTYPES; i++) bitsets_[i].clearAll();
}

void SelectionSet::setAll() {
  for (int i = 0; i < N_SELECTIONTYPES; i++) bitsets_[i].setAll();
}

std::vector<int> SelectionSet::size() const {
  std::vector<int> result(N_SELECTIONTYPES);
  for (int i = 0; i < N_SELECTIONTYPES; i++) result[i] = bitsets_[i].size();
  return result;
}

void SelectionSet::resize(std::vector<int> nbits) {
  for (int i = 0; i < N_SELECTIONTYPES; i++) bitsets_[i].resize(nbits[i]);
}
