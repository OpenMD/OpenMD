...
void SHAPES::calcForce(AtomType* at1, AtomType* at2, Vector3d d, RealType r,
                       ...
                       RotMat3x3d A1, RotMat3x3d A2, Vector3d& f1, Vector3d& t1,
                       Vector3d& t2) {
  if (!initialized_) initialize();
  ...
  if (i_is_LJ) {
    ...
    if (fabs(sti2) < 1.0e-12) {
      ...
      dcpidr = Vector3d(1.0_dp / proji - xi2 / proji3, -xi * yi / proji3, 0.0);
      dcpidu =
          Vector3d(xi / proji - (xi2 * xi) / proji3, -(xi * yi2) / proji3, 0.0);
      dspidr = Vector3d(-xi * yi / proji3, 1.0_dp / proji - yi2 / proji3, 0.0);
      dspidu =
          Vector3d(-(yi * xi2) / proji3, yi / proji - (yi2 * yi) / proji3, 0.0);
    }
