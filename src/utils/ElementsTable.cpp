...
ElementsTable::~ElementsTable() {
  ...
  for (i = elements_.begin(); i != elements_.end(); ++i) delete *i;
}
...
void ElementsTable::ParseLine(const char *buffer) {
  ...
  if (buffer[0] != '#') {
    sscanf(buffer,
           "%d %3s %lf %lf %*f %lf %d %lf %lf %lf %lf %lf %lf %lf %255s", &num,
           symbol, &ARENeg, &Rcov, &Rvdw, &maxbonds, &mass, &elNeg, &ionize,
           &elAffin, &red, &green, &blue, name);

    Element *ele = new Element(num, symbol, ARENeg, Rcov, Rvdw, maxbonds, mass,
                               elNeg, ionize, elAffin, red, green, blue, name);
...
void ElementsTable::Init() {
  ...
  if (ifs) {
    while (ifs.getline(charBuffer, BUFF_SIZE)) ParseLine(charBuffer);
  }
