...
namespace OpenMD {
  ...
  SeqRandNumGen::SeqRandNumGen(const uint32& oneSeed) {
    // In order to generate independent random number stream, the actual seed
    // used by random number generator is the seed passed to the constructor
    // plus the number of random number generators which are already created.
    int newSeed = oneSeed + nCreatedRNG_;
    mtRand_     = Utils::make_unique<MTRand>(newSeed, 1, 0);
    ++nCreatedRNG_;
  }

  SeqRandNumGen::SeqRandNumGen() {
    // the default seeds generated by MTRand will make sure they are unique
    mtRand_ = Utils::make_unique<MTRand>(1, 0);
    ++nCreatedRNG_;
  }
