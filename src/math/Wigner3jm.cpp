...
//                      satisfied. errflag=3  l1+l2+l3 not an integer. errflag=4
//                      m2max-m2min not an integer. errflag=5  m2max less than
//                      m2min. errflag=6  ndim less than m2max-m2min+1.
// Description:
// ------------
//
...
L200:
  ...
  for (n = 1; n <= nlim; ++n) thrcof[n] = ratio * thrcof[n];
...
L211:
  ++nlim;
  ratio = one / ratio;
  for (n = nlim; n <= nfin; ++n) thrcof[n] = ratio * thrcof[n];
...
L236:
  ...
  for (n = 1; n <= nfin; ++n) thrcof[n] = cnorm * thrcof[n];
  return;
