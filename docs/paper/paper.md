...
A relatively new algorithm for simulations of bulk metals, the density readjusting embedded atom method (`DR-EAM`), has also been implemented [@Bhattarai2019]. `DR-EAM` allows fluctuating densities within the framework of the Embedded Atom Method (EAM) [@Daw1984], by adding an additional degree of freedom, the charge for each atomic site. The total configurational potential energy, $U$, as a function of both instantaneous positions, $\{\textbf{r}\}$, and partial charges $\{q\}$:
