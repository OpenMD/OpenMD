...
`OpenMD` builds on the foundations of the Object-Oriented Parallel Simulation Engine (`OOPSE`) program [@Meineke2005]. Entirely rewritten in modern C++, it extends `OOPSE` in a number of key areas. The first of which is Reverse Non-Equilibrium Molecular Dynamics (RNEMD), a family of algorithms which impose a non-physical flux on a system and use linear response theory to compute transport properties as the system approaches steady state. The goal of RNEMD methods is to calculate the relevant transport property ($\lambda$) that connects the flux ($\textbf{J}$) and driving force ($\nabla X$) according to the generalized equation,

\begin{equation}\label{eq:linearConstitutive}
  \textbf{J} = - \lambda \nabla X.
\end{equation}

`OpenMD` is also capable of performing condensed phase simulations without the use of periodic boundary conditions. To do so, an external pressure and temperature bath is applied to atoms comprising the system's convex hull, rather than the interior region. This method, the `LangevinHull`, allows for constant pressure, temperature, or isobaric-isothermal (NPT) simulations of explicitly non-periodic molecular systems. Other major developments are the inclusion of advanced real-space electrostatics for point multipoles, and polarizable force fields using fluctuating charges or fluctuating electron densities.
...
One way to include the effects of polarizability in molecular simulations is the use of electronegativity equalization [@Rappe1991] or fluctuating charges on atomic sites [@Rick1994]. `OpenMD` makes it relatively easy to add extended variables (e.g. charges) on sites to support these methods. In general, the equations of motion are derived from an extended Lagrangian,
...
For simulations of bulk metals, a relatively new method, the density readjusting embedded atom method (`DR-EAM`) has also been implemented [@Bhattarai2019]. `DR-EAM` allows fluctuating densities within the framework of the Embedded Atom Method (EAM) [@Daw1984], by adding an additional degree of freedom, the charge for each atomic site. The total configurational potential energy, $U$, as a function of both instantaneous positions, $\{\textbf{r}\}$, and partial charges $\{q\}$:
...
`DR-EAM` was tested and validated against experimental results as well as DFT-based and `EAM` models for bulk metals which have little to no charge transfer between metal atoms. Common surfaces of the bulk metals like you may see on the surface of nanoparticles and ordered structures such as in alloys were also tested. Metal slabs in a uniform electric field or with a fixed external charge approaching the surface demonstrate the utility of a polarizable metal potential, as the image charge effect can be replicated in this model. The method was shown to perform well across all test systems, and most importantly, retain the strengths of unmodified `EAM` when it comes to bulk elastic constants and surface energies [@Bhattarai2019]. `DR-EAM` has similar performance to unmodified `EAM` modeling, as both approaches require a double pass through the force loop to first compute local densities, and a second pass to compute forces and energies.
